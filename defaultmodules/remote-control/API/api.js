...
module.exports = {
   ...
   */
  getApiKey () {
    const thisConfig = this.configOnHd.modules.find((x) => x.module === "remote-control");
  ...
  getExternalApiByGuessing () {
    ...
    const skippedModules = new Set(["clock", "compliments", "remote-control"]);
  ...
  createApiRoutes () {
    ...
    this.expressRouter.route("/classes/:value").
      get((request, response) => {
        const remoteControlConfig = this.configOnHd?.modules?.find((m) => m.module === "remote-control")?.config;
    ...
    this.expressRouter.route(route).
      get((request, response) => {
        ...
        switch (request.params.moduleName) {
          ...
          case "rc": case "RC": this.updateModule("remote-control", response); break;
