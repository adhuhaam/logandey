 ...
 * @param {string} moduleDirectory - Module directory path (usually __dirname from node_helper)
 * @returns {string} Absolute path to config.js
 */
function getConfigPath (moduleDirectory) {
  let configPath = path.resolve(`${moduleDirectory}/../../config/config.js`);
  if (globalThis.configuration_file !== undefined) {
    configPath = path.resolve(`${moduleDirectory}/../../${globalThis.configuration_file}`);
  }
  return configPath;
}
 ...
 */
function combineConfig (moduleDirectory, loadTranslationCallback) {
  // function copied from MagicMirrorOrg (MIT)
  const defaults = require(`${moduleDirectory}/../../js/defaults.js`);
  const configPath = getConfigPath(moduleDirectory);
  let thisConfig = {};
  let configOnHd;
  ...
  try {
    ...
    if ("modules" in configOnHd) {
      const thisModule = configOnHd.modules.find((m) => m.module === "remote-control");
...
function getConfig (configOnHd, configData) {
  ...
  for (const current of config.modules) {
    ...
    for (const key in moduleConfig) {
      if (!Object.hasOwn(current.config, key)) {
 ...
 */
function removeDefaultValues (moduleDirectory, config, configData) {
  ...
  let defaultConfig = {};
  try {
    const defaultsPath = `${moduleDirectory}/../../js/defaults.js`;
 ...
 */
async function loadTranslation (moduleDirectory, language, currentTranslation = {}) {
  try {
    const data = await fs.promises.readFile(path.resolve(`${moduleDirectory}/translations/${language}.json`), "utf8");
 ...
 */
async function loadDefaultSettings (moduleDirectory) {
  try {
    const data = await fs.promises.readFile(path.resolve(`${moduleDirectory}/settings.json`), "utf8");
 ...
 */
async function saveDefaultSettings (moduleDirectory, configData) {
  if (!configData?.moduleData) {
    Log.warn("Remote control: no mirror state to save yet.");
    return;
  }
  ...
  try {
    await fs.promises.writeFile(path.resolve(`${moduleDirectory}/settings.json`), text, "utf8");
  } catch (error) {
 ...
 */
async function loadCustomMenus (moduleDirectory, thisConfig, translateCallback) {
  if ("customMenu" in thisConfig) {
    const menuPath = path.resolve(`${moduleDirectory}/../../config/${thisConfig.customMenu}`);
    try {
