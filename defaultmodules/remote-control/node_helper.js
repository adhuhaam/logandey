...
module.exports = NodeHelper.create({
    ...
    if (defaultModules.includes(query.module)) {
      ...
      git.revparse(["HEAD"], (error, result) => {
        ...
        const githubPath = defaultModulesPath.includes("defaultmodules/defaultmodules.js")
          ? "defaultmodules"
          : "modules/default";
        response.writeHead(302, {"Location": `https://github.com/adhuhaam/logandey/tree/${result.trim()}/${githubPath}/${query.module}`});
        response.end();
      });
      return;
    }
  ...
  handleGetClasses (query, response) {
    ...
    const thisConfig = config.modules.find((m) => m.module === "remote-control")?.config || {};
    this.sendResponse(response, undefined, {
      query,
  ...
  callAfterUpdate (callback, timeout = 5000) {
    let isDone = false;
  ...
  requireLiveState (response, callback) {
    this.callAfterUpdate((didUpdate) => {
      if (didUpdate || this.initialized) {
        ...
        this.sendResponse(response, "Not connected to Lo Gandey. Make sure the mirror display is running on this device, then refresh the remote page.");
      }
    });
  },
  ...
  handleSave (query, response) {
    this.requireLiveState(response, async () => {
