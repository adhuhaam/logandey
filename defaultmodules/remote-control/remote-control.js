...
Module.register("remote-control", {
  ...
  defaults: {
    ...
    // When one of these modules is shown, hide all others (except exclusiveKeepVisible).
    exclusiveModules: [],
    exclusiveKeepVisible: ["remote-control"]
  },
  ...
  start () {
    ...
    // Keep the node helper connected even when this module has no on-screen UI.
    this.scheduleStatusSync();
  },

  /**
   * Periodically publish mirror state until the node helper acknowledges the link.
   */
  scheduleStatusSync () {
    const publishStatus = () => {
    ...
    if (this._statusSyncTimer) {
      clearInterval(this._statusSyncTimer);
    }

    let attempts = 0;
    this._statusSyncTimer = setInterval(() => {
      publishStatus();
      attempts += 1;
      if (attempts >= 36) {
        clearInterval(this._statusSyncTimer);
        this._statusSyncTimer = null;
      }
    }, 5000);
  },
  ...
  handleDefaultSettings (payload) {
    ...
    for (const moduleDatum of moduleData) {
      const lockStrings = Array.isArray(moduleDatum.lockStrings)
        ...
        : [];
      const hasRemoteControlLock = lockStrings.some((lockString) => lockString.includes("remote-control"));
  ...
  handleModuleVisibility (notification, payload) {
    ...
    this.applyExclusiveModuleMode(modules);
  },

  /**
   * When an exclusive module is visible, hide everything else.
   * When it is hidden, restore the normal mirror layout.
   * @param {Array} changedModules - Modules targeted by the visibility action
   */
  applyExclusiveModuleMode (changedModules) {
    const exclusiveModules = this.config.exclusiveModules ?? [];
    if (exclusiveModules.length === 0) {
      return;
    }

    const changedExclusive = changedModules.filter((module) => exclusiveModules.includes(module.name));
    if (changedExclusive.length !== 1) {
      return;
    }

    const [exclusiveModule] = changedExclusive;
    const keepVisible = new Set([
      ...exclusiveModules,
      ...(this.config.exclusiveKeepVisible ?? ["remote-control"])
    ]);
    const options = {lockString: this.identifier};

    MM.getModules().enumerate((module) => {
      if (exclusiveModule.hidden) {
        if (!keepVisible.has(module.name)) {
